...
Runs a battery of numerical checks on a structure, each against a route
...
``` r
check_covstruct(s, tol = 1e-06, verbose = TRUE)
```
...
wrong the structure is; saying so is the difference between a validator
and a formality.
...
#> Structure: scaled   (6 x 6, rank 4, 1 free)
