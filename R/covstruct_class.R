...
#'
#' @description
#' The abstract S7 class of covariance structures: a map from an unconstrained
...
#'
#' @details
#' A structure owns its dimension: \code{log_cholesky(dimension = 3)} and
...
#' \code{\link{struct_null_basis}}.
#'
#' Only \code{\link{struct_matrix}} is compulsory. Every other generic has a
#' numerical method registered on this class, so a new structure is a subclass
#' and one method, and a closed form supplied later replaces the corresponding
#' numerical method through dispatch.
#'
#' @param struct_name A single character string naming the family.
...
#' @param struct_params A list of whatever the family needs to evaluate itself.
#'
#' @return An object of class \code{covstruct}. The class is abstract; use one
#'   of the constructors, such as \code{\link{log_cholesky}}.
#'
#' @seealso \code{\link{log_cholesky}}, \code{\link{scaled_struct}},
#'   \code{\link{struct_matrix}}, \code{\link{check_covstruct}}
#'
#' @examples
#' s <- log_cholesky(3)
...
#'
#' @export
covstruct <- S7::new_class(
  "covstruct",
  properties = list(
    struct_name = S7::class_character,
...
#'
#' @keywords internal
check_struct_args <- function(dimension, role) {
...
#' structure declares, and returns it unnamed.
#'
#' @details
...
check_eta <- function(s, eta) {
  ...
  if (length(eta) != s@n_free) {
    stop(sprintf(
      ...
      length(eta), s@struct_name, s@n_free,
...
#'
#' @description
#' Checks that \code{m} is a square symmetric numeric matrix of the structure's
#' dimension.
#'
#' @param s A \code{\link{covstruct}} object.
...
check_matrix <- function(s, m, tol = 1e-8) {
  ...
  if (!identical(dim(m), c(s@dimension, s@dimension))) {
    stop(sprintf(
      "'m' must be %d by %d, matching the structure.", s@dimension, s@dimension
    ), call. = FALSE)
  }
...
#'
#' @export
struct_null_basis <- function(mats, tol = 1e-10) {
