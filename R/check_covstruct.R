...
#' spreading a log-Cholesky structure over twenty-eight units of log gives a
#' condition number around \eqn{10^{28}} -- and a comparison that fails there
#' is a statement about the arithmetic rather than about the structure. The
...
#'
#' @description
#' Runs a battery of numerical checks on a structure, each against a route the
...
#' the structure is; saying so is the difference between a validator and a
#' formality.
#'
#' The checks are:
#' \enumerate{
...
#'     from an eigendecomposition rather than from \code{\link{struct_solve}}.
...
#'   \code{statistic}.
#'
#' @seealso \code{\link{struct_is_numerical}}
#'
#' @examples
#' invisible(check_covstruct(log_cholesky(3)))
#'
#' # a rank-deficient penalty passes the same battery
#' invisible(check_covstruct(scaled_struct(crossprod(diff(diag(6), differences = 2)))))
#'
#' @export
check_covstruct <- function(s, tol = 1e-6, verbose = TRUE) {
  if (!S7::S7_inherits(s, covstruct)) {
    stop("'s' must inherit from class 'covstruct'.", call. = FALSE)
  }
  num <- struct_is_numerical(s)
  etas <- sweep_etas(s)
  ...
  # --- 2. the round trip ----------------------------------------------------
  has_free <- !inherits(
    tryCatch(struct_free(s, struct_matrix(s, etas[[1L]])), error = function(e) e),
    "error"
  )
  if (!has_free) {
  ...
  if (num[["struct_d2matrix"]] || num[["struct_dmatrix"]]) {
    out[[length(out) + 1L]] <- check_row("second derivatives", "NOT CHECKED")
  } else {
    err <- 0
    for (eta in etas[seq_len(min(3L, length(etas)))]) {
      a <- struct_d2matrix(s, eta)
      b <- numerical_d2matrix(s, eta)
      for (k in seq_along(a)) err <- max(err, rel(a[[k]], b[[k]]))
    }
    out[[length(out) + 1L]] <- check_row(
  ...
  if (num[["struct_logdet"]]) {
    ...
    for (eta in etas) {
      m <- struct_matrix(s, eta)
      ev <- eigen((m + t(m)) / 2, symmetric = TRUE, only.values = TRUE)$values
      err <- max(err, rel(struct_logdet(s, eta), sum(log(ev[seq_len(s@rank)]))))
    }
    out[[length(out) + 1L]] <- check_row(
  ...
  if (num[["struct_dlogdet"]] || s@n_free == 0L) {
    ...
    for (eta in etas) {
      ...
      err <- max(err, rel(unname(struct_dlogdet(s, eta)), unname(want)))
    }
    out[[length(out) + 1L]] <- check_row(
  ...
  if (num[["struct_d2logdet"]] || num[["struct_dlogdet"]] || s@n_free == 0L) {
    ...
    for (eta in etas[seq_len(min(3L, length(etas)))]) {
      ...
      for (i in seq_along(idx)) {
        ...
        up[l] <- eta[l] + h
        dn[l] <- eta[l] - h
        want <- (struct_dlogdet(s, up)[[k]] - struct_dlogdet(s, dn)[[k]]) / (2 * h)
  ...
  if (s@rank < s@dimension) {
    ...
    for (eta in etas) {
      err <- max(err, rel(
        struct_solve(s, eta), solve(struct_matrix(s, eta))
      ))
      l <- struct_factor(s, eta)
      err <- max(err, rel(tcrossprod(l), struct_matrix(s, eta)))
    }
    out[[length(out) + 1L]] <- check_row(
  ...
  shape_ok <- length(s@free_names) == s@n_free &&
    identical(names(struct_dmatrix(s, eta)), s@free_names) &&
    identical(names(struct_d2matrix(s, eta)), struct_pair_names(s)) &&
    length(struct_pair_names(s)) == s@n_free * (s@n_free + 1L) / 2L &&
  ...
  if (verbose) {
    cat(sprintf(
      "Structure: %s   (%d x %d, rank %d, %d free)\n",
      s@struct_name, s@dimension, s@dimension, s@rank, s@n_free
    ))
