...
test_that("the constructor validates its matrix", {
  expect_error(scaled_matrix(1:4), "numeric matrix")
  expect_error(scaled_matrix(matrix(1:6, 2, 3)), "square")
  expect_error(scaled_matrix(matrix(c(1, 2, 3, 4), 2, 2)), "symmetric")
  expect_error(scaled_matrix(matrix(NA_real_, 2, 2)), "missing")
  expect_error(scaled_matrix(diag(c(1, -1))), "positive semidefinite")
  expect_error(scaled_matrix(matrix(0, 3, 3)), "positive semidefinite")

  # a link whose range is not the positive half line
  expect_error(
    scaled_matrix(diag(3), link = linkfunctions7::identity_link()),
    "positive half"
  )
})
...
test_that("the rank and the null space are those of the fixed matrix", {
  ...
  expect_identical(scaled_matrix(diag(4))@rank, 4L)
  expect_identical(dim(scaled_matrix(diag(4))@null_basis), c(4L, 0L))
})
...
test_that("the matrix is the fixed one times the scale", {
  ...
  for (eta in c(-2, 0, 1.5)) {
    expect_equal(param_value(s, eta), exp(eta) * p, ignore_attr = TRUE)
  }
})
...
test_that("the log pseudo-determinant is the rank times the log scale", {
  ...
  for (eta in c(-3, 0, 2.5)) {
    expect_equal(param_logdet(s, eta), logdet_plus(param_value(s, eta)))
    expect_equal(
      param_logdet(s, eta),
      r * eta + logdet_plus(p)
    )
...
test_that("the derivative of the log pseudo-determinant is the rank", {
  ...
  for (eta in c(-6, -1, 0, 4, 9)) {
    expect_equal(unname(param_dlogdet(s, eta)), as.numeric(s@rank))
    h <- 1e-5
    fd <- (param_logdet(s, eta + h) - param_logdet(s, eta - h)) / (2 * h)
    expect_equal(unname(param_dlogdet(s, eta)), fd, tolerance = 1e-8)
    expect_equal(unname(param_d2logdet(s, eta)), 0)
  }
})
...
test_that("the constant is what makes the scale estimable", {
  ...
  # and the constant in question is exactly the parameter's log-determinant
  eta <- log(num)
  expect_equal(param_logdet(s, eta) - param_logdet(s, 0), s@rank * eta)
})
...
test_that("the derivatives are the fixed matrix times the link's own", {
  ...
  s <- scaled_matrix(p)
  for (eta in c(-1, 0.7)) {
    expect_equal(param_d1(s, eta)[[1L]], exp(eta) * p, ignore_attr = TRUE)
    expect_equal(param_d2(s, eta)[[1L]], exp(eta) * p, ignore_attr = TRUE)
  }

  # against finite differences, which share no code with the closed form
  n1 <- parameters7:::numerical_d1(s, 0.3)
  expect_equal(param_d1(s, 0.3)[[1L]], n1[[1L]], tolerance = 1e-6)
})

test_that("a rank-deficient parameter refuses to be inverted", {
  s <- scaled_matrix(pen2(6))
  expect_error(param_solve(s, 0), "rank deficient")
  expect_error(param_factor(s, 0), "rank deficient")

  # a full-rank one does not
  expect_silent(param_solve(scaled_matrix(diag(3)), 0))
})

test_that("a fixed matrix has no free value at all", {
  s <- scaled_matrix(diag(3), link = NULL)

  expect_identical(s@n_free, 0L)
  expect_identical(s@free_names, character(0))
  expect_identical(s@param_name, "fixed")
  expect_equal(param_value(s, numeric(0)), diag(3), ignore_attr = TRUE)
  expect_length(param_d1(s, numeric(0)), 0L)
  expect_length(param_d2(s, numeric(0)), 0L)
  expect_length(param_dlogdet(s, numeric(0)), 0L)
  expect_equal(param_logdet(s, numeric(0)), 0)
  expect_length(param_tuple_names(s), 0L)

  # and a free vector is refused, since there is none to supply
  expect_error(param_value(s, 0), "0 free value")
})

test_that("the round trip recovers the scale and refuses another matrix", {
  p <- pen2(6)
  s <- scaled_matrix(p)
  expect_equal(unname(param_free(s, 3.7 * p)), log(3.7))
  expect_error(param_free(s, pen2(6) + diag(6)), "not a multiple")
  expect_error(param_free(s, -p), "positive multiple")
})

test_that("the ridge is the identity, scaled", {
  s <- scaled_matrix(diag(4))
  expect_identical(s@rank, 4L)
  expect_equal(param_logdet(s, 1.2), 4 * 1.2)
  expect_equal(param_value(s, log(3)), diag(3, 4), ignore_attr = TRUE)
})
